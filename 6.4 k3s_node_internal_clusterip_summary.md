...
## 5) NodePort/External 트래픽에서 중요한 SNAT(Masquerade)

Service가 ClusterIP만 있을 때는 비교적 단순하지만,  
**NodePort/LoadBalancer처럼 외부에서 들어오는 트래픽**에서는 경로 이슈가 생길 수 있습니다.

### 5-1. “응답이 어디로 돌아가야 하지?” 문제

외부 클라이언트 → NodePort(노드IP:30080) → Pod 로 들어오고  
Pod가 응답할 때 라우팅이 꼬이면 연결이 깨질 수 있습니다.

그래서 kube-proxy는 상황에 따라 **SNAT(마스커레이드)** 를 적용합니다.

- **SNAT**: 출발지(`src`)를 노드IP로 변경  
  → 응답이 노드로 확실히 돌아오게 해 경로 안정성↑  
  → 대신 **클라이언트 원본 IP가 보존되지 않을 수 있음**

### 5-2. `externalTrafficPolicy=Local`

원본 클라이언트 IP 보존을 위해 자주 쓰는 옵션입니다.

- `Local`: **해당 노드에 Pod가 있을 때만** 트래픽을 받아 처리  
  (다른 노드로 넘기지 않음)
- 장점: 원본 IP 보존에 유리
- 단점: 노드별 Pod 존재 여부에 따라 트래픽 편중/드롭 가능
