...
즉, 이 전체 명령에서 **리눅스 자체 명령은 tail과 파이프(|)**이고,
events는 Kubernetes의 Event 리소스를 의미합니다.
```
---
```
...
NAME: events
리소스의 plural 이름(=kubectl get events에서 쓰는 이름)
...
events.k8s.io/v1은 외부 k8s.io에 접속한다는 뜻이 아니라,
내 클러스터 API 서버 안에 있는 “events.k8s.io”라는 API 그룹을 쓴다는 의미
```
---
...
kubectl top pod -A
```
### kubectl top nodes = 노드들(node들) 의 CPU/메모리 사용량을 표로 보여줌 - 노드가 여러 대면 여러 줄로 전부 나옵니다

---
### 위의 top 명령어는 linux 자체 명령어로 실시간으로 CPU/메모리 사용량, 프로세스 목록을 갱신해서 보여줍니다.
### 종료: q
```
ubuntu@cp1:~$ top
top - 01:21:31 up  5:20,  2 users,  load average: 1.20, 0.84, 0.72
Tasks: 158 total,   1 running, 157 sleeping,   0 stopped,   0 zombie
%Cpu(s):  0.0 us, 14.3 sy,  0.0 ni, 78.6 id,  0.0 wa,  0.0 hi,  7.1 si,  0.0 st
MiB Mem :   3912.1 total,    734.9 free,   1074.4 used,   2102.9 buff/cache
MiB Swap:   2253.0 total,   2252.0 free,      1.0 used.   2547.9 avail Mem

    PID USER      PR  NI    VIRT    RES    SHR S  %CPU  %MEM     TIME+ COMMAND
   1864 root      20   0 2171272 705392 157992 S  20.0  17.6  35:00.66 k3s-server
      1 root      20   0  102180  13192   8248 S   0.0   0.3   0:04.81 systemd
